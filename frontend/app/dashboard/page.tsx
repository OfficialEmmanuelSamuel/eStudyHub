...
export default function DashboardPage() {
  ...
  // Fetch platform summary with real-time polling
  useEffect(() => {
    let mounted = true;
  ...
  const progressPercent =
    summary.topics > 0
      ? Math.min(100, Math.round((overview.stats.topicsDone / summary.topics) * 100))
      : 0;
      ...
      >
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:col-span-2 xl:col-span-3">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Progress: {progressPercent}% ({overview.stats.topicsDone}/{summary.topics || 0} topics)
          </p>
      ...
      >
        <article className="p-6">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            ] as [string, string, IconType][]).map(([k, v, Icon], idx) => (
              <div
                key={k}
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg text-center mb-2 font-semibold text-slate-900">eStudyHub Leaderboard</h2>
          ...
          <div className="mt-4 space-y-2 md:hidden">
            {users.map((u, idx) => (
              ...
              >
                <p className="text-xs text-slate-500">Rank #{start + idx + 1}</p>
                <p className="text-sm font-semibold text-slate-900">{u._name}</p>
